...
console.log(baseDeDatos.results);
...
function renderizarDatosUsuario(datos) {
    ...
    const bdd = JSON.parse(datos)
    const contenedor = document.querySelector('.tarjeta');
    contenedor.innerHTML += `<img src="" alt="Foto de Perfil">
                            <h2>${bdd.name} ${bdd.name}</h2>
                            <p>${bdd.email}</p>
    `
}
renderizarDatosUsuario(baseDeDatos.results);